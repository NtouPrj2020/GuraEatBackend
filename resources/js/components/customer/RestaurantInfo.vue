<template>
    <div>
        ...
        >
            ...
            >
                <v-card class="mx-auto mt-10">
                    <v-img :src="list.img" class="white--text align-end" height="200px">
                        <v-card-title>
                            <div
                ...
                <v-overlay :value="overlay">
                  ...
                  ></v-img>
                            ...
                            <v-list-item v-for="(dishes, index) in menu" :key="index">
                                    ...
                                    <v-row dense>
                                        <v-col>
                                            <v-btn
                                                ...
                                                @click="
                          increaseNumber(index, dishes.id);
                          $forceUpdate();
                        "
                                                icon
                                            >
                                            ...
                                            <v-btn
                                                color=""
                                                @click="
                          decreaseNumber(index, dishes.id);
                          $forceUpdate();
                        "
                                                icon
                                            >
                        ...
                        <v-card-text>
                            <div class="mt-2 mb-16">
                                ...
                                <v-row class="rounded mx-1" style="background-color: #cceeff">
                                    ...
                                    <v-spacer></v-spacer>
                                    <v-col cols="3"> ${{ order.totalAmount }}</v-col>
                                </v-row>
                                ...
                                <v-card color="#CCEEF">
                                    <v-toolbar flat color="#26c6da">
                                        ...
                                        <v-spacer></v-spacer>
                                        <v-btn color="orange" fab small @click="EditInfo">
                                            <v-icon v-if="isEditing" @click="cDeliveryTime()">
                                                mdi-close
                                            </v-icon>
                                            <v-icon v-else> mdi-account-edit-outline</v-icon>
                                        </v-btn>
                                    </v-toolbar>
                                    <v-card-text>
                                        <!--<v-text-field
        ...
        <div class="text-center">
            <v-dialog v-model="isCorrectDialog" width="500">
                <v-card>
                    ...
                    <v-card-actions>
                        ...
                        <v-btn
                            ...
                            text
                            @click="
                CustomerInfo.address = tempAddress;
                isCorrectDialog = false;
              "
                        >
                            是
                        </v-btn>
                        <v-btn color="primary" text @click="isCorrectDialog = false">
                            否
                        </v-btn>
                    </v-card-actions>
                </v-card>
...
export default {
    ...
    mounted() {
        ...
        customerGetRestaurantByIDAPI(config)
            .then((res) => {
    ...
    methods: {
        ...
        getDish() {
            ...
            customerGetAllDishByRestaurantIDAPI(config)
                .then((res) => {
        ...
        getTotalAmount() {
            ...
            for (let i = 0; i < this.order.id.length; i++) {
                if (this.order.amount[i] > 0) {
                    ...
                    if (this.menu[i].making_time > this.MakingTime)
                        this.MakingTime = this.menu[i].making_time;
                }
            }
            if (this.isOrderNull === true) this.$emit("showSnackBar", "請選擇餐點");
            else {
        ...
        success(position) {
            ...
            customerlocationToAddressAPI(config)
                .then((res) => {
        ...
        cDeliveryTime() {
            ...
            customerGetDeliveryTimeIDAPI(config)
                .then((res) => {
                    console.log("addressToaddress");
                    console.log(res.data);
                    this.durationTime.text =
                        res.data.data.rows[0].elements[0].duration.text;
                    this.durationTime.text = this.durationTime.text.substring(
                        0,
                        this.durationTime.text.length - 5
                    );
                    this.durationTime.value =
                        res.data.data.rows[0].elements[0].duration.value;
                    this.orderDistance.text =
                        res.data.data.rows[0].elements[0].distance.text;
                    this.orderDistance.text = this.orderDistance.text.substring(
                        0,
                        this.orderDistance.text.length - 3
                    );
                    this.orderDistance.value =
                        res.data.data.rows[0].elements[0].distance.value;
        ...
        TotalDeliveryTime() {
            let temp;
            if (
                this.durationTime.value + this.MakingTime >
                this.durationTime.text * 60
            )
                temp = parseInt(this.durationTime.text, 10) + 1;
